...
def train_inverse(train_loader, model, criterion, attack, optimizer, epoch, half=False, print_freq=50, double=False, adv=None):
    """
        Run one train epoch
    """
    batch_time = AverageMeter()
    ...
    for i, (input_data, target) in enumerate(train_loader):
        ...
        target = torch.tensor([0] * batch_size + [1] * batch_size, dtype=torch.float32).cuda()
        input_var = input_data.cuda()
        target_var = target.reshape(-1, 1)
        if half:
            input_var = input_var.half()
        if double:
            input_var = input_var.double()
        ...
        if adv:
            input_var = adv(model, input_var, target_var)
        noises = torch.rand([batch_size, 32 * 32])
        noisy_images = attack(model, noises, torch.tensor([0] * batch_size, dtype=torch.float32).reshape(-1, 1)).view(batch_size, 1, 32, 32)
        input_var = torch.vstack([input_var, noisy_images])
        # compute output
        output = model(input_var)
        loss = criterion(output, target_var)
        # compute gradient and do SGD step
